...
Реальный запрос выполняет backend через официальный Python SDK `OpenAI().responses.create(...)`, передавая `model`, `instructions`, `input` и `store=False`. Список данных ограничен профилем одного сотрудника, первыми пятью рекомендациями и последними 10 парами вопрос/ответ из его истории. Ответы из истории ограничиваются 6000 символами на реплику; актуальные карьерные данные имеют приоритет перед устаревшими ответами. Вопрос обрезается до 1000 символов; на API действует базовый throttle DRF. Для production следует добавить корпоративную авторизацию, отдельный лимит на AI запросы, аудит доступа и политику хранения данных.
...
Успешные вопросы и ответы хранятся в таблице `ChatTurn` локальной SQLite или PostgreSQL. Примените миграции командой `python manage.py migrate` после обновления. История сохраняется после перезапуска приложения; API-провайдер не используется как хранилище (`store=False`). При ошибке провайдера незавершённая пара не записывается, а интерфейс возвращает вопрос в поле ввода.

В разделе AI наставника можно загрузить предыдущие сообщения и очистить всю историю выбранного сотрудника с подтверждением. После очистки старые реплики не передаются в следующие запросы модели. В демо выбор сотрудника открыт всем участникам; для production доступ к истории необходимо привязать к авторизованному сотруднику.

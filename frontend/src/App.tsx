    ...
    <div className="workspace">
      ...
      {!employeeId ? <Selector t={t} choose={choose}/> :
        ...
        snap ? <Routes>
          ...
          <Route path="/coach" element={<Coach snap={snap} t={t} lang={lang}/>}/>
...
function Coach({snap,t,lang}:{snap:Snapshot;t:T;lang:Lang}) {const [question,setQuestion]=useState('');const [messages,setMessages]=useState<{from:'user'|'coach';text:string}[]>([]);const [busy,setBusy]=useState(false);const send=async(text:string)=>{if(!text.trim())return;setMessages(old=>[...old,{from:'user',text}]);setQuestion('');setBusy(true);try{const r=await api<{answer:string}>('/ai/chat/',{method:'POST',body:JSON.stringify({employee_id:snap.employee.id,message:text,language:lang})});setMessages(old=>[...old,{from:'coach',text:r.answer}])}catch(e){setMessages(old=>[...old,{from:'coach',text:String(e)}])}finally{setBusy(false)}};return <main className="page coach-page"><Heading eyebrow={t('coach')} title={t('coach')}/><div className="chat"><div className="chat-messages">{messages.length===0&&<div className="chat-intro"><span className="coach-symbol">✳</span><h2>{t('ask')}</h2><button onClick={()=>send(t('question'))}>{t('question')} ↗</button></div>}{messages.map((m,i)=><div className={`bubble ${m.from}`} key={i}>{m.text}</div>)}{busy&&<div className="bubble coach">{t('loading')}</div>}</div><p className="disclaimer">{t('disclaimer')}</p><form className="chat-composer" onSubmit={e=>{e.preventDefault();void send(question)}}><input value={question} onChange={e=>setQuestion(e.target.value)} placeholder={t('ask')} aria-label={t('ask')}/><button className="primary" disabled={busy}>{t('send')}</button></form></div></main>}

...
The engine takes plain dataset records and effective skills, returns pure gap/recommendation/roadmap calculations, and does not call an LLM. `TemplateAIProvider` formats a transparent explanation; future OpenAI/NVIDIA providers may explain engine output but must not choose or reorder recommendations. The UI shows loading, error, and empty states, including no career target and no eligible activities. AI chat is a visual placeholder.
